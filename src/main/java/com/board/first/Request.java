...
public class Request {
    ...
    public void updateUrl(String input) {
        if (input == null || input.isBlank()) {
            throw new BoardAppException(ErrorCode.COMMAND_NOT_URL);
        }

        if (!input.startsWith("/")) {
            throw new BoardAppException(ErrorCode.COMMAND_INVALID_URL);
        }
        ...
        if (pathSegments.length < 2) {
            throw new BoardAppException(ErrorCode.COMMAND_INVALID_FORM);
        }
